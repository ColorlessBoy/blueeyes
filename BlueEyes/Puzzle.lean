...
def World (n : Nat) := Fin n → EyeColor

abbrev WProp (n : Nat) := World n → Prop

abbrev Model (n : Nat) := World n → Prop

def indistinguishable (i : Fin n) (w v : World n) : Prop :=
  ∀ j, j ≠ i → v j = w j

def Knows (M : Model n) (i : Fin n) (w : World n) (P : WProp n) : Prop :=
  ∀ v, M v → indistinguishable i w v → P v

/-- The proposition that agent `i` is red-eyed. -/
def IsRed (i : Fin n) : WProp n :=
  fun world => world i = EyeColor.red

/-- The proposition that at least one agent is red-eyed. -/
def AtLeastOneRed (world : World n) : Prop :=
  ∃ i, world i = EyeColor.red

/-- The unconstrained starting model: every color assignment is possible. -/
def initialModel : Model n :=
  fun _ => True

/-- Publicly announcing `A` restricts the current model to worlds satisfying `A`. -/
def publicAnnouncement (M : Model n) (P : WProp n) : Model n :=
  fun world => M world ∧ P world

/-- The model immediately after the guru announces that at least one person is red-eyed. -/
def afterGuruAnnouncement : Model n :=
  publicAnnouncement initialModel AtLeastOneRed

/-- The guru announcement removes exactly the worlds with no red-eyed agent. -/
theorem afterGuruAnnouncement_iff {w : World n} :
...
def WouldLeave (M : Model n) (i : Fin n) (w : World n) : Prop :=
  Knows M i w (IsRed i)

/-- A quiet night is one in which no agent knows that they are red-eyed. -/
def QuietNight (M : Model n) : WProp n :=
...
-/
def afterQuietNights : Nat → Model n
...
theorem afterQuietNights_zero :
    afterQuietNights (n := n) 0 = afterGuruAnnouncement :=
  rfl

/-- The successor step is one more public update by the fact that nobody left. -/
theorem afterQuietNights_succ (n : Nat) :
    afterQuietNights (n := n) (n + 1) =
      afterOneQuietNight (afterQuietNights (n := n) n) :=
  rfl

/-- Recolor only agent `i`, leaving all other observations unchanged. -/
def recolorSelf (i : Fin n) (c : EyeColor) (world : World n) :
    World n := fun j => if j = i then c else world j

/-- Recoloring only one's own eyes is observationally invisible to that agent. -/
theorem indistinguishable_recolorSelf (i : Fin n) (c : EyeColor)
    (w : World n) : indistinguishable i w (recolorSelf i c w) := by
  intro j hj
  simp [recolorSelf, hj]

/-- Recoloring oneself to red makes `IsRed i` true in the recolored world. -/
theorem isRed_recolorSelf_red (i : Fin n) (w : World n) :
...
theorem not_isRed_recolorSelf_green (i : Fin n) (w : World n) :
...
-/
theorem not_knows_red_if_green_variant_possible {M : Model n}
    {i : Fin n} {w : World n} (hgreen : M (recolorSelf i EyeColor.green w)) :
    ¬ Knows M i w (IsRed i) := by
  intro hknow
  apply not_isRed_recolorSelf_green i w
  apply hknow (recolorSelf i EyeColor.green w) hgreen
  apply indistinguishable_recolorSelf i EyeColor.green w

/-- Knowledge is monotone with respect to logical implication inside a model. -/
theorem Knows.mono {M : Model n} {i : Fin n} {w : World n} {P Q : WProp n}
...
def EveryoneKnows (M : Model n) (w : World n) (P : WProp n) : Prop :=
  ∀ i, Knows M i w P
...
-/
def IteratedEveryoneKnows (M : Model n) (P : WProp n) :
    Nat → WProp n
  | 0 => P
...
-/
def CommonKnowledge (M : Model n) (P : WProp n) : WProp n :=
  fun w => ∀ n, IteratedEveryoneKnows M P n w

/-- Common knowledge implies the announced fact itself. -/
theorem CommonKnowledge.base {M : Model n} {P : WProp n} {w : World n}
...
-/
theorem commonKnowledge_unfold {M : Model n} {P : WProp n} {w : World n} :
...
-/
def updateModelByReport (M : Model n) (present report : Fin n → Prop) : Model n :=
...
def nextNightState (S : NightState n) (actual : World n) : NightState n where
...
theorem updateModelBy_empty_report_eq_quiet {M : Model n} :
    updateModelByReport M (fun _ : Fin n => True) (fun _ => False) =
...
theorem seenRed_eq_of_indistinguishable {i : Fin n} {w v : World n}
    (hobs : indistinguishable i w v) :
...
theorem totalRed_eq_seenRed_add_one_of_red {i : Fin n} {w : World n}
    (hred : IsRed i w) :
...
theorem totalRed_eq_seenRed_of_not_red {i : Fin n} {w : World n}
    (hgreen : ¬ IsRed i w) :
...
plus one.
-/
theorem compatibleWithConcreteObservation (i : Fin n) (w : World n) :
...
theorem totalRed_pos_of_atLeastOneRed {w : World n} (h : AtLeastOneRed w) :
    0 < totalRed w := by
  rcases h with ⟨i, hi⟩
...
theorem afterQuietNights_count_lower_bound :
    ∀ n {w : World Agent}, afterQuietNights n w → n < totalRed w
  | 0, w, hw => by
  ...
  | n + 1, w, hw => by
      ...
      have hleave : WouldLeave (afterQuietNights n) i w := by
        intro v hvM hobs
        by_contra hgreen
...
theorem redAgent_wouldLeave_after_seenQuietNights
    {i : Fin n} {w : World n} (_hred : IsRed i w)
    (_hw : afterQuietNights (seenRed i w) w) :
    WouldLeave (afterQuietNights (seenRed i w)) i w := by
  intro v hvM hobs
  by_contra hgreen
  have hseen : seenRed i v = seenRed i w :=
    seenRed_eq_of_indistinguishable hobs
...
theorem allRedAgents_wouldLeave_on_decisiveNight
    {w : World n} (hw : afterQuietNights (totalRed w - 1) w) :
    ∀ i, IsRed i w → WouldLeave (afterQuietNights (totalRed w - 1)) i w := by
  intro i hred
...
def concreteCountingView (i : Fin n) (w : World n) :
    CountingView where
  seen := seenRed i w
  totalRed := totalRed w
...
-/
theorem possibleAfterQuietNights_of_concrete
    {quietNights : Nat} {i : Fin n} {w : World n}
...
-/
theorem possibleAfterQuietNights_of_afterQuietNights
    {n : Nat} {i : Fin n} {w : World n}
    (hw : afterQuietNights n w) :
...
-/
theorem possibleAfterConcreteRedSeenQuietNights
    {i : Fin n} {w : World n} (hred : IsRed i w) :
...
-/
theorem redEyedPerson_knows_concrete_total
    {i : Fin n} {w : World n} {candidateTotal : Nat} (hred : IsRed i w)
    (hpossible :
...
def twoRedOneGreen : World 3 := fun i => if i = 0 then EyeColor.red else if i = 1 then EyeColor.red else EyeColor.green
...
theorem twoRedOneGreen_after_one_quiet_night :
    afterQuietNights 1 twoRedOneGreen := by
  constructor
...
theorem twoRedOneGreen_reds_leave_after_one_quiet_night
    (hw : afterQuietNights 1 twoRedOneGreen) :
    WouldLeave (afterQuietNights 1) (0 : Fin 3) twoRedOneGreen ∧
      WouldLeave (afterQuietNights 1) (1 : Fin 3) twoRedOneGreen := by
  constructor
...
theorem twoRedOneGreen_reds_leave_together :
    WouldLeave (afterQuietNights 1) (0 : Fin 3) twoRedOneGreen ∧
      WouldLeave (afterQuietNights 1) (1 : Fin 3) twoRedOneGreen :=
...
def allRedTwo : World 2 := fun _ => EyeColor.red

/-- The world obtained from `allRedTwo` by making islander `0` green. -/
def greenRedTwo : World 2 := fun i => if i = 0 then EyeColor.green else EyeColor.red

/-- The world in which neither of the two islanders is red-eyed. -/
def allGreenTwo : World 2 := fun _ => EyeColor.green
...
-/
def greenPrefixFive (n : Nat) : World 5 := fun i => if i < n then EyeColor.green else EyeColor.red

/-- The concrete world in which all five islanders are red-eyed. -/
def allRedFive : World 5 :=
  greenPrefixFive 0

/-- The concrete world in which all five islanders are green-eyed. -/
def allGreenFive : World 5 :=
  greenPrefixFive 5
